...
var os = require('os');
var RT = require('./rt');
//var RxPlugin = require('weex-rax-webpack-plugin');
module.exports = {
    configPathExists: function () {
        if (!fs.existsSync(RT.configPath)) {
            console.log('当前目录rax-config.js 不存在，请切换正确目录');
            process.exit(0)
        }
    },
    handExternal: function (config,src) {
        var b = false;
        var externals = config.externalModules || {};
        //weex 特殊逻辑
        if(/^@weex\-module\//.test(src)){
            b = src;
        }else if(typeof externals === 'function'){
            //有返回值表示命中
            var url = externals(src) ;
            url && (b = url);
            //只支持简单数组
        }else if(Array.isArray(externals)){
            externals.indexOf(src) !== -1 && (b = src) ;
        }else if(externals instanceof RegExp){
            externals.test(src) && (b = src);
        }else if(externals && typeof externals === 'object'){
            externals[src] ? (b = externals[src]) : Object.keys(externals).indexOf(src) !==-1 && (b = src);
    ...
    getPlugins: function (defaultWebpackOption, options) {
        ...
        if(options.isAmdExternal === true){
            var AmdPlugin = require('amd-externals-plugin');
            plugins.push(new AmdPlugin({
                externals:  function(context, request, callback) {
                    var url = that.handExternal(options,request);
                    if (url) {
                        return callback(null, url);
                    }
                    callback();
                },
                wrap: options.isWeb === true? null:function (obj) {
                    var frameworkComment = options.frameworkComment || '// {"framework" : "Rax"}\n';
                    var addRequire =  options.isAddRequire === false ? '': ';require('+obj.name+')';
                    var arrOutput = obj.arrOutput;
                  ...
                  //  arrOutput[0] = "define(" + obj.name + ", " + obj.externalsDepsArray + ", function("+factoryDependencies.join(',')+") { module.exports = ";
                    arrOutput.unshift(frameworkComment);
                    arrOutput.push(addRequire);
                    return arrOutput
                }
            }));
        }
    ...
    getWebpackConfig: function (RX) {
        ...
        if(options.isWeb !== true){
            options.isAmdExternal = true;
        }
        if(options.externalModules){
            options.isAmdExternal = true;
        }
        return new Promise(function (reslove) {
            ...
            var babel = util._extend({
                ...
                presets: [options.isWeb=== true? require('babel-preset-react'):require('babel-preset-rax'), require('babel-preset-es2015'), require('babel-preset-stage-0')]
            },options.babel);
            var webpackConfig = {
                output: output,
                resolve: {
