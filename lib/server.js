...
var qrTemplate = options.isWeb === true ? '':  fs.readFileSync(path.join(__dirname,'../template/qr.js'),'utf8').replace(/\$IP/g,ip);
//处理 html
app.use(ctoolHtmlMiddleware(cwdPath, {
  ...
  transform:  function(content){
    ...
    if(options.isWeb !== true){
      content = content.replace(/<\s*\/\s*body\s*>/i,'<script type="text/javascript">'+qrTemplate+'</script></body>')
    }
    return content
