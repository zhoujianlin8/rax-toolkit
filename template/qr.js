(function (global) {
    ...
    var mod = {
        init: function function_name (argument) {
            //qrhide=1
            if(location.search.indexOf('qrhide=1') !== -1) return;
            this.view();
            this.bindEvent();
        },
        view: function(){
            this.$node  = document.createElement('div');
            var qrWeex = 'http:'+'//$IP:'+(location.port || 80)+location.pathname.replace(/\.htm(l)?$/g,'.js')+location.search+location.hash;
            var qrH5 = 'http:'+'//$IP:'+(location.port || 80)+location.pathname+(location.search ? location.search+'&qrhide=1': '?qrhide=1' )+location.hash;
            this.$node.innerHTML = '<span title="click me show qr" class="JJ_toggle" href="javascript:" style="width: 20px;height: 20px; display: block;cursor: pointer; border-radius: 100%; background-size: 15px 15px;background-repeat: no-repeat;  background-position: center; box-shadow: 0 0 0px 1px #999;"></span>'
                +'<div class="JJ_content" style="display:none; border: 1px solid #999; position: absolute;left: 0;text-align: center; top: 0;margin-top: -170px; z-index: 10000">'
                +'<div style="line-height: 18px;"><a href="javascript:" class="JJ_weex" style="text-decoration: underline;">weex</a><a href="javascript:" class="JJ_h5" style="padding : 0 5px; text-decoration: none;">h5</a>QR</</div>'
                +'<div class="JJ_weexQR" title="'+qrWeex+'"><canvas></canvas></div>'
                +'<div class="JJ_h5QR" title="'+qrH5+'"><canvas></canvas></div>'
                +'</div>';
            this.$node.style.position = 'fixed';
            this.$node.style.bottom = '10px';
            this.$node.style.left = '10px';
            new QRious({
                element: this.$node.querySelector('.JJ_weexQR').querySelector('canvas'),
                size: 140,
                value: qrWeex
            });
            new QRious({
                element: this.$node.querySelector('.JJ_h5QR').querySelector('canvas'),
                size: 140,
                value: qrH5
            });

            this.$node.querySelector('.JJ_h5QR').style.display = 'none';
            this.$node.querySelector('.JJ_toggle').style.backgroundImage = "url(data:image/png;base64,)";

            document.body.append(this.$node)
        },
        toggleContent: function(b){
            var $content = this.$node.querySelector('.JJ_content');
        ...
        bindEvent: function(){
            var $node = this.$node;
            var that = this;
            $node.querySelector('.JJ_toggle').addEventListener('click',function(e){
                that.toggleContent();
            },false);

            $node.querySelector('.JJ_weex').addEventListener('click',function(){
                $node.querySelector('.JJ_weexQR').style.display = 'block';
                $node.querySelector('.JJ_weex').style.textDecoration = 'underline';

                $node.querySelector('.JJ_h5QR').style.display = 'none';
                $node.querySelector('.JJ_h5').style.textDecoration = 'none';

            },false);

            $node.querySelector('.JJ_h5').addEventListener('click',function(){
                $node.querySelector('.JJ_weexQR').style.display = 'none';
                $node.querySelector('.JJ_weex').style.textDecoration = 'none';

                $node.querySelector('.JJ_h5QR').style.display = 'block';
                $node.querySelector('.JJ_h5').style.textDecoration = 'underline';
